    ...
    return (
        <div className="relative pb-4">
            ...
            {isLoadingTypes ? (
                ...
                isOpen && (
                    <div
                        className="absolute left-0 mt-1 w-full max-h-[250px] overflow-y-auto rounded-md border bg-white shadow-lg">
                        {types?.filter(type => !selectedTypes.some(st => st.id === type.id))
                            .filter(type => type.name.toLowerCase().includes(searchInput.toLowerCase()) || searchInput === '')
                            .map((type) => (
                                <div
                                    key={type.id}
