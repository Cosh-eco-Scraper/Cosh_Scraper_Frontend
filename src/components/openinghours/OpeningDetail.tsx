...
export default function OpeningDetail({ openingHour }: OpeningDetailProps) {
  ...
  const isMainPeriodClosed =
    openingAt?.toLowerCase() === 'closed' || closingAt?.toLowerCase() === 'closed';

  if (isMainPeriodClosed) {
    return (
      <tr>
        {/* Apply source-serif-pro-semi-bold and font-bold as defined in your global CSS or Tailwind config */}
        <td className="source-serif-pro-semi-bold font-bold capitalize">{day}</td>
        <td className="source-serif-italic-text pr-4 text-right font-bold capitalize">Closed</td>
        <td></td>
  ...
  return (
    <tr onClick={() => console.log(openingHour)}>
      ...
      <td className="font-inter-normal pr-4 text-right">
        ...
        <span className="font-source-serif-pro-semi-bold">{formattedMorningOpening.time}</span> -{' '}
        {formattedMorningClosing.hour}{' '}
        <span className="font-source-serif-pro-semi-bold">{formattedMorningClosing.time}</span>
      </td>

      {hasAfternoonHours ? (
        <td className="font-inter-normal pr-4 text-right">
          {formattedAfternoonOpening.hour}{' '}
          <span className="font-source-serif-pro-semi-bold">{formattedAfternoonOpening.time}</span>{' '}
          - {formattedAfternoonClosing.hour}{' '}
          <span className="font-source-serif-pro-semi-bold">{formattedAfternoonClosing.time}</span>
        </td>
      ) : (
