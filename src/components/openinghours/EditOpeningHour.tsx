...
export function EditOpeningHour({
    ...
    const CreateNewHour = (opening: string, closing: string, openingAn: string | null, closingAn: string | null) => {
        return {
            id,
            day,
            openingAt: opening,
    ...
    const updateOpening = (time: string) => {
        const hour = CreateNewHour(time, closingAt, openingAtAfterNoon ?? null, closingAtAfterNoon ?? null);
        updateHour(id, hour)
    }
    const updateClosing = (time: string) => {
        const hour = CreateNewHour(openingAt, time, openingAtAfterNoon ?? null, closingAtAfterNoon ?? null);
        updateHour(id, hour)
    }
    ...
    return (
        <tr>
            <td className="px-4 py-2 text-left text-xs font-medium tracking-wider text-gray-500 uppercase">{day}</td>
            ...
            <td className="px-4 py-2 text-center text-xs font-medium tracking-wider text-gray-500 uppercase">
                <TimeInput time={closingAt} isClosed={isClosed} onChange={updateClosing}/>
            </td>
            <td>
                {
                    (openingAtAfterNoon &&
                        <TimeInput time={openingAtAfterNoon} isClosed={isClosed} onChange={updateOpeningAfternoon}/>)
                }
            ...
            <td>
                {
                    (closingAtAfterNoon &&
                        <TimeInput time={closingAtAfterNoon} isClosed={isClosed} onChange={updateClosingAfternoon}/>)
                }
        </td>
    </tr>
    )
